...
class TaskLogger:
    def __init__(self, csv_file='task_log.csv'):
        ...
        with open(self.csv_file, mode='w', newline='') as file:
            ...
            writer.writerow(['Task ID', 'Start Position', 'End Position', 'Robot', 'Start Time', 'End Time', 'Duration (s)'])

    def log_task(self, task_data: dict):
        task_id = task_data['booking']['id']
        robot = task_data['assigned_to']['name']
        start_time = task_data['unix_millis_start_time']

        if task_id not in self.task_records:
            start_position = task_data['phases']['1']['category']
        ...
        if task_data['status'] == 'completed':
            end_position = task_data['phases']['2']['category'] if '2' in task_data['phases'] else 'Unknown'
            end_time = task_data['unix_millis_finish_time']
            duration = (end_time - start_time) / 1000  # Convert to seconds
            self.task_records[task_id]['end_position'] = end_position
            self.task_records[task_id]['end_time'] = end_time
            self.task_records[task_id]['duration'] = duration
            self.write_to_csv(task_id)

    def write_to_csv(self, task_id: str):
        task_record = self.task_records[task_id]
        with open(self.csv_file, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([
                task_id,
                task_record['start_position'],
                task_record['end_position'],
                task_record['robot'],
